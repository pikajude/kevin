...
sendUserList us uss rm = do
    ...
    where names        = map (\u -> T.concat [T.pack . show . levelToSym $ privclassLevel u, username u]) uss
          chunkedNames = reverse . map reverse . subchunk' 432 names $ [[]]
          subchunk' n  = fix (\f x y ->
                             let hy = head y
                                 hx = head x
                                 ty = tail y
                                 tx = tail x
...
sendWhoisReply me us rn rooms idle signon = do
    ...
    sendPacket $ printf "%s 317 %s %s %s %s :seconds idle, signon time"
        [hostname, me, us, T.pack $ show idle, T.pack $ show signon]
    sendPacket $ printf "%s 318 %s %s :End of /WHOIS list." [hostname, me, us]

data Mode = None | Voice | Operator | Founder
