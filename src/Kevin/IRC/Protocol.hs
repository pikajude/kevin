...
respond pkt "JOIN" = do l <- gets_ (view loggedIn)
                        if l
...
respond pkt "PRIVMSG" = do let (room:msg:_) = pkt^.params
                           if "\1ACTION" `T.isPrefixOf` msg
                              then do let newMsg = T.drop 8 $ T.init msg
                                      D.sendAction room $ entityEncode newMsg
                              else D.sendMsg room $ entityEncode msg

respond pkt "MODE" = if length (pkt^.params) > 1
                        then do let (toggle,mode) = first (=="+") . T.splitAt 1 $ pkt^.params.ix 1
                                case mode of "b" -> if' toggle
                                                        D.sendBan
                                                        D.sendUnban
                                                        (pkt^.params._head)
...
respond pkt "TOPIC" = case pkt^.params of []             -> sendNotice "Malformed packet"
                                          [room]         -> D.sendGet room "topic"
                                          (room:topic:_) -> D.sendSet room "topic" topic

respond pkt "TITLE" = case pkt^.params of []           -> sendNotice "Malformed packet"
                                          [room]       -> do title <- gets_ . view $ titles.ix room
...
respond pkt "NAMES" = do let room = pkt^.params._head
                         k <- get_
...
respond pkt "PROMOTE" = case pkt^.params of (room:user:group:_) -> D.sendPromote room user $ Just group
...
unmask y = case T.split (`elem` "@!") y of [s] -> Just s
...
notice h str = do klogNow Blue ("client -> " ++ T.unpack asStr)
                  T.hPutStr h (asStr <> "\r\n")
