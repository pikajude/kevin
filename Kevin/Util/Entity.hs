...
lookupHexEntity e = case hexadecimal $ T.cons '0' e of
    Right (n,_) -> do
        guard $ inRange validCharRange n
        return $ chr $ fromInteger n
    Left _ -> Nothing

lookupNumericEntity :: T.Text -> Maybe Char
lookupNumericEntity e = case decimal e of
    Right (n,_) -> do
        guard $ inRange validCharRange n
        return $ chr $ fromInteger n
    Left _ -> Nothing

validCharRange :: (Integer, Integer)
validCharRange = (toInteger $ ord minBound, toInteger $ ord maxBound)
