...
respond _ "dAmnServer" = do s <- use_ settings
                            sendLogin (s^.name) (s^.authtoken)

respond pkt "login" = if okay pkt
                         then do j <- kevin $ do loggedIn .= True
                                                 use joining
                                 mapM_ sendJoin j
                         else I.sendNotice $ "Login failed: " <> pkt ^. args.ix "e"

respond pkt "join" = do roomname <- deformatRoom $ pkt^.parameter._Just
                        if okay pkt
                           then do kevin $ joining %= (roomname:)
                                   uname <- use_ name
                                   I.sendJoin uname roomname
                           else I.sendNotice $ T.concat ["Couldn't join ", roomname, ": ", pkt^.args.ix "e"]

respond pkt "part" = do roomname <- deformatRoom $ pkt^.parameter._Just
                        if okay pkt
                            then do uname <- use_ name
                                    removeRoom roomname
                                    I.sendPart uname roomname Nothing
                            else I.sendNotice $ T.concat ["Couldn't part ", roomname, ": ", pkt^.args.ix "e"]

respond pkt "property" = do roomname <- deformatRoom (pkt^.parameter._Just)
                            case pkt^.args.ix "p" of "privclasses" -> setPrivclasses roomname . parsePrivclasses $ pkt^.body._Just

                                                     "topic" -> do uname <- use_ name
                                                                   I.sendTopic uname roomname (fromMaybe uname $ pkt^.args.at "by")
                                                                                              (T.replace "\n" " - " . entityDecode . tablumpDecode $ pkt^.body._Just)
                                                                                              (pkt^.args.ix "ts")
...
respond spk "recv" = deformatRoom (spk^.parameter._Just) >>=
    \roomname -> case pkt^.command of "join" -> do let usname = pkt^.parameter._Just
                                                   pcs <- gets_ $ view privclasses
                                                   countUser <- numUsers roomname usname
                                                   let us = mkUser roomname pcs modifiedPkt
                                                   addUser roomname us
                                                   if countUser == 0
                                                      then do I.sendJoin usname roomname
                                      ...
                                      "part" -> do let uname = pkt^.parameter._Just
                                      ...
                                      "kicked" -> do let uname = pkt^.parameter._Just
                                                     removeUserAll roomname uname
                                                     I.sendKick uname (arg "by") roomname $ pkt^.body.traverse ^? notNull_

                                      "admin" -> case pkt^.parameter._Just of "create"    -> I.sendRoomNotice roomname $ T.concat [ "Privclass ", arg "name"
                                                                                                                                  , " created by ", arg "by"
                                                                                                                                  , " with: ", arg "privs" ]
                                                                              "update"    -> I.sendRoomNotice roomname $ T.concat [ "Privclass ", arg "name"
                                                                                                                                  , " updated by ", arg "by"
                                                                                                                                  , " with: ", arg "privs" ]
                                                                              "rename"    -> I.sendRoomNotice roomname $ T.concat [ "Privclass ", arg "prev"
                                                                                                                                  , " renamed to ", arg "name"
                                                                                                                                  , " by ", arg "by" ]
                                                                              "move"      -> I.sendRoomNotice roomname $ T.concat [ arg "n", " users in privclass "
                                                                                                                                  , arg "prev", " moved to "
                                                                                                                                  , arg "name", " by ", arg "by" ]
                                                                              "remove"    -> I.sendRoomNotice roomname $ T.concat [ "Privclass", arg "name"
                                                                                                                                  , " removed by ", arg "by" ]
    ...
    where pkt         = fromJust $ subPacket spk
          modifiedPkt = parsePacket (T.replace "\n\npc" "\npc" $ readable pkt)
          arg s       = pkt^.args.ix s

respond pkt "kicked" = do roomname <- deformatRoom $ pkt^.parameter._Just
                          uname <- use_ name
                          removeRoom roomname
