...
sendChanAction sender room msg = mapM_ (\x ->
    sendPacket $ printf "%s PRIVMSG %s :\1ACTION %s\1"
        [getHost sender, room, x]
    ) . T.splitOn "\n" $ msg

sendKick kickee kicker room msg =
    sendPacket $ printf "%s KICK %s %s%s"
        [getHost kicker, room, kickee, maybeBody msg]

sendTopic us rm maker top startdate = do
    sendPacket $ printf "%s 332 %s %s :%s" [hostname, us, rm, top]
    sendPacket $ printf "%s 333 %s %s %s %s"
        [hostname, us, rm, maker, startdate]
...
sendUserList us uss rm = do
    mapM_ (\nms ->
        sendPacket $ printf "%s 353 %s = %s :%s"
            [hostname, us, rm, T.unwords nms]) chunkedNames
    sendPacket $ printf "%s 366 %s %s :End of /NAMES list." [hostname, us, rm]
    where
        names = map (\u ->
            T.concat [levelToSym $ privclassLevel u, username u]) uss
...
sendNoticeClone uname i rm =
    sendPacket $ printf "%s NOTICE %s :%s has joined again\
         \ (now joined %s times)" [hostname, rm, uname, T.pack $ show i]

sendNoticeUnclone uname i rm =
    sendPacket $ printf "%s NOTICE %s :%s has parted (now joined %s)"
        [hostname, rm, uname, times]
    where
        times | i == 1    = "once"
              | otherwise = T.pack (show i) <> " times"

sendWhoisReply me us rn rooms idle signon = do
    sendPacket $ printf "%s 311 %s %s %s chat.deviantart.com * :%s"
        [hostname, me, us, us, rn]
    sendPacket $ printf "%s 307 %s %s :is a registered nick" [hostname, me, us]
    sendPacket $ printf "%s 319 %s %s :%s"
        [hostname, me, us, T.intercalate " " . map (T.cons '#') $ rooms]
    sendPacket $ printf "%s 312 %s %s chat.deviantart.com :dAmn"
        [hostname, me, us]
